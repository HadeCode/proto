...
try:
    from argus_ml import FlowFeatureExtractor, get_ml_engine
    _HAVE_ML = True
except ImportError:
    _HAVE_ML = False
...
class ArgusOneDetector:
    ...
    augmented by an ensemble ML classifier (Random Forest & Gradient Boosted Trees)
    and benign anomaly guard.
    """
    def __init__(self, config: DetectorConfig | None = None, baseline: MetadataBaseline | None = None) -> None:
        ...
        self.ml_engine = get_ml_engine() if _HAVE_ML else None
    ...
    def process_flow(self, raw: Mapping[str, Any] | Flow) -> list[dict[str, Any]]:
        ...
        with self._lock:
            ...
            emitted = [f for f in findings if self._emit(f, flow.timestamp)]
            if not emitted:
                return []
            ...
            for f in emitted:
                ...
                if self.ml_engine is not None:
                    try:
                        window_sec = max(60, f.window_seconds)
                        recent = self.store.window(flow.timestamp, window_sec, lambda flow_item: flow_item.src_ip == flow.src_ip or flow_item.dst_ip == flow.dst_ip)
                        target_flow = {
                            "timestamp": flow.timestamp, "src_ip": flow.src_ip, "dst_ip": flow.dst_ip,
    ...
    def _alert(self, finding: Finding, timestamp: float, ml_pred: Any = None) -> dict[str, Any]:
        ...
        if ml_pred is not None:
            alert["ml_confidence"] = round(ml_pred.confidence, 4)
            alert["ml_predicted_class"] = ml_pred.threat_class
